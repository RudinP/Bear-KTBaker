...
export function buildIosCss(project: ThemeProject, template: string) {
  ...
  // The 250204 guide uses openchats + View; its sample CSS still uses view.
  // Keep both declarations alongside now. Piccoma is a separate Japanese tab.
  for (const state of ['normal', 'selected'] as const) {
    const suffix = state === 'selected' ? 'Selected' : '';
    for (const key of ['now', 'openchats', 'view'] as const) {
      css = replaceInBlock(
        css,
        'TabBarStyle-Main',
        `-ios-${key}-${state}-icon-image`,
        `'maintabIco${key === 'now' ? 'Now' : 'View'}${suffix}.png'`,
      );
    }
  }
