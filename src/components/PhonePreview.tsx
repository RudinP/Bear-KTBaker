...
function PasscodeScreen(props: PreviewProps) {
  ...
  return <div className="kt-screen kt-passcode" style={screenStyle(project, platform, 'passcode')} onClick={() => onSelect('screen-background')}>
    ...
    <div className="kt-passcode-title" style={{ top: passcode.titleTop, color: cssColor(colorValue(project, platform, 'passcode.foreground')) }}><ColorHotspot slotId="passcode.foreground" selected={selected} onSelect={onSelect} className="kt-passcode-title-copy"><span><h4>{platform === 'ios' ? '비밀번호' : '암호'}</h4><p>카카오톡 암호를 입력해주세요.</p></span></ColorHotspot><div className="kt-passcode-bullets" role="button" tabIndex={0} aria-label="잠금화면 상태별 이미지 꾸미기" onClick={(event) => { event.stopPropagation(); onSelect('passcode-keypad'); }}>{[1, 2, 3, 4].map((index) => { const state = index <= entered ? 'selected' : 'normal'; const image = resolveResourceUrl(project, platform, `passcode.bullet.${index}.${state}`); return image ? <img key={index} data-testid={`passcode-bullet-${index}`} data-passcode-bullet={index} data-state={state} src={image} alt="" /> : <i key={index} data-testid={`passcode-bullet-${index}`} data-passcode-bullet={index} data-state={state} />; })}</div></div>
