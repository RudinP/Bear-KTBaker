        ...
        <div className="patch-canvas" ref={area} style={{
          ...
          {platform === 'android' ? <>
            ...
            {(['x', 'y'] as const).flatMap((axis) => {
              ...
              return [
                <button key={`${axis}-start`} aria-label={`늘림 기준 ${axis.toUpperCase()} 가이드`} className={`guide ${orientation}`} data-guide-kind="stretch" data-ios-inset-guide={`stretch-${axis}-start`} data-source-px={start} data-px={`${iosStretchLogicalPixel(axis)}px · 원본 ${start}px`} style={position} onPointerDown={(event) => dragIosStretchPoint(axis, event)} />,
                <span key={`${axis}-end`} aria-hidden="true" className={`guide ${orientation} derived`} data-guide-kind="stretch" data-ios-inset-guide={`stretch-${axis}-end`} data-source-px={end} data-px={`원본 ${end}px`} style={derivedPosition} />,
              ];
            })}
            {(['left', 'right'] as const).map((edge) => <button key={edge} aria-label={`글자 ${edge === 'left' ? '왼쪽' : '오른쪽'} 가이드`} className="guide content vertical" data-guide-kind="content" data-ios-inset-guide={`content-${edge}`} data-px={`${iosInsetPixel(edge)}px · 원본 ${pixel('x', activeGuides.content[edge])}px`} style={{ left: `${activeGuides.content[edge] * 100}%` }} onPointerDown={(event) => dragContent(edge, event)} />)}
            {(['top', 'bottom'] as const).map((edge) => <button key={edge} aria-label={`글자 ${edge === 'top' ? '위' : '아래'} 가이드`} className="guide content horizontal" data-guide-kind="content" data-ios-inset-guide={`content-${edge}`} data-px={`${iosInsetPixel(edge)}px · 원본 ${pixel('y', activeGuides.content[edge])}px`} style={{ top: `${activeGuides.content[edge] * 100}%` }} onPointerDown={(event) => dragContent(edge, event)} />)}
          </>}
        </div>
